...
### 6.2. Luồng Tác giả: Sáng tác và Đăng chương (Author Flow)
```mermaid
graph LR
    Start[Trang chủ] --> Login[Đăng nhập]
    Login --> Dash[Dashboard Tác giả]
    Dash --> Manage[Quản lý truyện]
    Manage --> Editor[Trình soạn thảo]
    Editor --> AI[Sử dụng Trợ lý AI]
    AI --> Editor
    Editor --> Settings[Cài đặt chương: Giá/Hẹn giờ]
    Settings --> Publish[Xuất bản]
```

### 6.3. Luồng Quản trị viên: Kiểm duyệt nội dung (Admin Flow)
```mermaid
graph LR
    AdminLogin[Login Admin] --> AdminDash[Dashboard Quản trị]
    AdminDash --> Queue[Danh sách chờ duyệt/Bị báo cáo]
    Queue --> Detail[Xem chi tiết nội dung]
    Detail --> Decision{Ra quyết định}
    Decision -- Hợp lệ --> Approve[Phê duyệt & Hiển thị]
    Decision -- Vi phạm --> Action[Gỡ bỏ/Yêu cầu sửa]
    Action --> Notify[Gửi thông báo cho Tác giả]
```
